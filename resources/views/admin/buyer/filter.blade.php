		...
		<h2 class="text-base text-gray-700 font-semibold">{{trans('details.buyer')}} </h2>
		<a href="{{url('/admin/buyer/add')}}" class="bg_orange text-white text-sm px-4 py-1 rounded-full">{{trans('details.createbuyer')}}</a>
		</div>
		<div class="my-3"><form method="GET"   class="mb-0">
      ...
      <div class="relative sort_by">
       ...
       <select name="sort_by" class="block appearance-none w-32 lg:w-40  border custom_border py-1 px-4 pr-8 rounded-full  focus:outline-none focus:bg-white focus:outline-none text-xs" id="sort_by" onchange="change()">
          ...
          <option value="asc"  {{request()->get('sort_by')=="asc"? 'selected':''}}>{{trans('details.ascending')}}</option>
          <option value="desc" {{ request()->get('sort_by')=="desc"? 'selected':''}}>{{trans('details.decending')}}</option>
        </select>
        <div class="pointer-events-none absolute inset-y-0 right-0 flex items-center px-2 ">
          <svg class="fill-current h-4 w-4 select_icon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20"><path d="M9.293 12.95l.707.707L15.657 8l-1.414-1.414L10 10.828 5.757 6.586 4.343 8z"/></svg>
					...
					<div class="relative mx-3">
						<input type="text" id="search" value="{{$search}}" name="search" placeholder="Search" class="text-xs border custom_border rounded-full  py-1 focus:outline-none px-4 w-32 lg:w-40">
						<img src="{{url('images/icons_1/search.svg')}}" class="w-3 h-3 absolute top-0 right-0 m-2">
					</div>
					</div>
