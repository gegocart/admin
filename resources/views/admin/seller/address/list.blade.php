...
@include('admin.seller.success')
		...
		<div class="flex items-center justify-between pt-2 pb-4 border-b mb-5" style="border-color: 1px solid #f3f3f3;">
		<h2 class="text-base text-gray-700 font-semibold">{{trans('edituser.addresses')}}</h2>
		...
		<div>
			<table class="w-full">
				<thead>
					<tr>
							<th>{{trans('edituser.firstname')}}</th>
							<th>{{trans('edituser.lastname')}}</th>
							<th>{{trans('edituser.mobileno')}}</th>
							<th>{{trans('edituser.address')}}</th>
							<th>{{trans('edituser.city')}}</th>
							<th>{{trans('edituser.state')}}</th>
							<th>{{trans('edituser.country')}}</th>
							<th>{{trans('edituser.pincode')}}</th>
							<th>{{trans('edituser.default')}}</th>
							<th>{{trans('edituser.action')}}</th>
	...
	@foreach($addresses as $address)
		...
		<!-- 	<td class="px-3 py-3"><input type="checkbox"></td> -->
			<td class="px-3 py-3">{{$address->firstname}}</td>
			<td class="px-3 py-3">{{$address->lastname}}</td>
			<td class="px-3 py-3">{{$address->mobileno}}</td>
			<td class="px-3 py-3">{{$address->address_1.','.$address->address_2}}</td>
			<td class="px-3 py-3">{{$address->city->name}}</td>
			<td class="px-3 py-3">{{$address->state->name}}</td>
			<td class="px-3 py-3">{{$address->country->name}}</td>
			<td class="px-3 py-3">{{$address->postal_code}}</td>
			<td class="px-3 py-3">
 	...
 	<li>
			 <a href="{{url('/admin/seller/'.$userid.'/address/'.$address->id.'/edit')}}">
...
<li>
	...
	<a  class="del"  rel="{{url('/admin/seller/address/'.$address->id.'/delete')}}">
...
@endforeach
		...
		<div class="custom-paginate py-2">
			{{ $addresses->appends(['search' =>request()->get('search')])->links() }}
...
function change()
{
    ...
    window.location.href="{{url('/admin/seller/'.$userid.'/addresses?sort_by=')}}"+$sort_by+"&paginate="+$paginate+"&search="+$search;
}
