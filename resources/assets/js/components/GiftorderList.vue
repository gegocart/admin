...
<li>
 <!--  <a v-on:click="showPopup(order.to_email,order.id)" href="#">
...
<a :href="'/admin/giftorder/'+order.order_id+'/show'">
       ...
       <div class="modal-body">
            ...
            <div class="modal-header flex justify-between items-center border-b px-4 py-1">
             ...
             <button class="modal-default-button text-2xl py-1"  id="close_modal" v-on:click="closePopup()">
               ...
               <div class="py-6 px-4">
              <label class="form-group block my-1 custom-label text-sm">Email</label>
          <input type="email"  v-model="email"  class="border text-sm px-2 py-1 w-full mb-2">
    ...
    export default {
      ...
      methods:{
        getOrder(page =1)
        {
          ...
          axios.post('/admin/giftorder/getlist?&page='+page,{
    ...
    .then((willupdate) => {
      if (willupdate) {
        ...
        statusUpdate(){
          this.success=null;
          this.errors=[];
          axios.post('/admin/giftorder/'+this.id+'/update',{
            email:this.email,
          }).then(response=>{
              this.success=response.data.messages;
